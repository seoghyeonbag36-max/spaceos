...
예:
```text
/page 가로수길 공실 히트맵을 네이버 지도 위에 색상으로 표시하고, 건물 클릭 시 과거 3년 업종 변천사 패널을 띄워줘
```

### 3.2 하루의 시작 — 오케스트레이션
```text
/pppp-status 오늘은 가로수길 B2B 데모 준비에 집중
```
...
/posting 가로수길 1층 공실에 고급화/가성비/기능중심 3축 ROI 시뮬레이션 카드 만들어줘
```
...
```text
data-engineer 서브에이전트로 가로수길 네이버플레이스 리뷰를 data/bronze/platform/reviews 에 수집해줘.
...
## 6. 팀 상권 확장 — 수집 재현성(수집 메타 매니페스트) ★리드 판단 시 도입

> **맥락** — 팀원 5명이 각자 상권을 분석해 브랜치로 올리고, 리드가 주1회 취합한다(팀원 가이드: `SpaceOS_Team_Member_District_Guide.md`).
> 데이터(`data/bronze`·`data/gold`)는 git 제외라 **원본은 옮기지 않고 리드가 중앙에서 재수집**하는 것이 기본(방법 A). 이때 아래를 도입하면 재수집이 정확·재현 가능해진다.

### 6.1 문제
bronze/gold가 git에서 제외되므로, 리드가 재수집할 때 **"어떤 상권을·어떤 TRDAR 코드로·어느 분기로 수집했고, 팀원이 어떤 수기 판단(대체 매핑·보정)을 했는지"** 근거가 각자 laptop에만 남아 흩어진다. 상권이 수십·수백으로 늘면 재현이 모호해진다.

### 6.2 해결 — 상권별 작은 매니페스트만 git에 추적
무거운 데이터는 계속 제외하되, 상권별 **작은 `manifest.json`**(수집 상권·TRDAR 코드·분기·수집일·수기 메모)만 추적한다. 리드는 이 매니페스트만 보고 **정확히 동일 조건으로 재수집**할 수 있다.

- `.gitignore` 예외 한 줄: `!data/bronze/**/manifest.json`
- `/district-add` 가 상권 추가 시 매니페스트를 자동 생성하도록 확장.

### 6.3 도입 명령 (필요하다고 판단되면 레포 루트에서 입력)
```
data/bronze 는 계속 git 제외를 유지하되, 상권별 수집 메타만 버전관리하도록 도입해줘.

1. .gitignore 에 예외 추가: !data/bronze/**/manifest.json  (기존 data/bronze/* 제외 규칙은 유지)
2. 매니페스트 스키마 정의: { district_id, district_name, gu, trdar_cd[], quarters[],
   collected_at, collector_versions, manual_notes[], proxy_todo[] }.
   data/config 또는 data/bronze/{id}/manifest.json 경로 규약을 CONTRIBUTING.md 에 1문단으로 기록.
3. .claude/commands/district-add.md 를 수정해, 상권 추가 시 data/bronze/{id}/manifest.json 을
   자동 생성/갱신하는 단계를 STEP 2~3 사이에 넣어줘(수집 실패 시에도 계획된 코드·분기는 기록).
4. 기존 13거점(platform13)에 대해 현재 platform_districts.py 매핑을 근거로 매니페스트를 소급 생성.
5. cd apps/backend && pytest 통과 확인 후, chore 브랜치로 커밋·PR.
```

### 6.4 도입 후 주간 취합에 미치는 변화
- 팀원 PR에는 `manifest.json`이 포함돼 온다 → 리드는 매니페스트 목록만 보고 **재수집 대상·조건을 한눈에** 파악.
- 재수집: `refresh_platform` 실행 전, 매니페스트의 `trdar_cd`·`quarters` 가 `platform_districts.py`와 일치하는지 Claude에 교차검증 요청.
- **도입 시점 판단:** 팀원·상권 수가 적고 재수집이 단순하면 불필요(기본 방법 A로 충분). **재현이 모호해지거나 수기 판단이 늘 때** 도입한다.
