...
⚠ **라벨 어휘 3안(a/b/c)은 서로 McNemar 로 비교할 수 없다**(2026-09-16). 전제가
  깨지기 때문이다: `group`(7종+미분류) · `group_mapped`(7종만, 모집단이 더 작다) ·
  `lcls`(대분류 10종)는 **라벨이 다르므로 분할이 다르고**, off-prior 자리 자체도
  거점 사전분포가 달라 다르게 잡힌다. 그래서 이 스크립트를 세 안에 돌리면 위 집합
  검사가 **정상적으로** 에러를 낸다 — 그건 고칠 결함이 아니라 맞는 답이다.
  교집합만 남겨 비교하면 안 된다(남는 자리가 두 어휘 중 하나에 유리하게 치우친다).
  세 안은 쌍대 비교가 아니라 **각자의 거점사전 기준선 대비**로 읽는다
  (finding-map-provider-google-2026-09-15.md §7-2-3-1 의 표).
  같은 어휘 안에서 피처·설정만 바꾼 두 팔은 종전대로 이 스크립트가 맞다.
