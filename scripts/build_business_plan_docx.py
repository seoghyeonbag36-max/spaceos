...
def cover(doc):
    ...
    run(p, "SpaceOS", size=42, bold=True, color=NAVY, spacing=-20)
...
def sec0(doc):
    ...
    body(doc,
         "먼저 규칙을 밝힌다. 본문의 모든 수치는 SpaceOS가 직접 만든 산출물에서 인용했다. "
...
def sec1(doc):
    ...
    body(doc,
         "이 네 단계를 하나의 데이터 위에서 잇는 것이 SpaceOS의 설계 목표다. 그 구조가 3장의 PPPP 다.")
    bullets(doc, [
...
def sec2(doc):
    ...
    body(doc,
         "SpaceOS는 물리적 상권을 하나의 플랫폼으로 읽는 디지털 트윈이다. 가설은 'Place ▶ Platform' — "
    ...
    body(doc,
         "상권 분석 도구는 이미 여럿 있다. SpaceOS가 다른 지점은 읽기에서 끝나지 않는다는 것이다. "
...
def sec4(doc):
    ...
    note_box(doc, [
        ("세 거점 모두 SpaceOS 값이 앵커보다 높다. 폭은 +5.52%p ~ +7.5%p 다. "
    ...
    bullets(doc, [
        ...
        ("서열 보존. ", "SpaceOS 기준 순위는 신촌·이대(17.2%) > 홍대(15.0%) > 연남동(12.5%) 이고, "
...
def sec6(doc):
    ...
    body(doc,
         "SpaceOS 는 진행률을 문서가 아니라 산출물로 센다. 거점 하나가 '완료'라는 말은 건축물대장 실측이 "
...
def appendix(doc):
    ...
    run(p, "이 목록이 짧아지는 속도가 곧 SpaceOS 의 진행 속도다.", size=9.5, bold=True, color=NAVY)


def build_document(out_path: Path) -> Path:
    doc = Document()
    sec = setup(doc)
    add_footer(sec, "SpaceOS 사업 기획안 · 투자자용")
    cover(doc)
    sec0(doc)
    sec1(doc)
    sec2(doc)
...
def main() -> int:
    ...
    ap.add_argument("-o", "--out", default="SpaceOS_사업기획안_투자자용.docx")
