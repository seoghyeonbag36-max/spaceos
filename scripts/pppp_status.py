...
`docs/placeos-vibe-build-sequence.md` 는 두 번 낡았다 — 08-02 에 멈춰 Tier1 을 13거점으로,
...
def page_track(total: int) -> Track:
    ...
    t.gates.append(Gate(
        ...
        "⚠ **2026-09-06 재검증 — 근거가 변경보다 이르러 다시 쟀다.** 이 줄은 그날까지 "
        "`/verify 2026-09-05 (커밋 fd04852)` 를 근거로 걸려 있었는데, 그 뒤 프론트가 "
        "**세 번 더** 바뀌었고 그중 217d469 는 지도 생명주기 자체를 갈아엎었다"
        "(MapHost 로 지도를 앱 하나로 승격 · 탭 전환은 visibility · 오버레이 수거). "
        "217d469 자신의 검증 목록은 **카메라·경계·hubBoundary 불변식**이고 이 게이트가 "
        "말하는 **건물 패널·층 스택·거리뷰는 거기 없다** — 즉 선언이 참인지 아무도 "
        "안 본 상태였다. 실제로 띄워 확인했다(가로수길): 지도 캔버스 1376×900 "
        "visible·SDK 자식 2 · 거점 옵션 **66** · `.b-item` **840**(payload features 와 "
        "일치) · 탭 왕복 후 캔버스·목록 그대로(접힘 없음) · 동남빌딩 패널에서 층 스택 "
        "**7행 3색**(공실 5 · 영업 1 · 층 미상 1) · 거리뷰 파노라마 실렌더 + "
        "촬영일 2026-01-20 과 '공실 근거 아님' 문구 동반 · pageerror 0. "
        "⚠ 같이 드러난 것: `.b-name` 이 목록행과 상세패널 **양쪽**에 있어 "
        "`querySelector('.b-name')` 는 목록 첫 행을 집는다 — 프로브는 "
        "`.b-detail .b-name` 로 좁혀야 한다(패널 자체는 정상이었다)",
        auto=False,
        evidence=("apps/frontend/src/pages/MapShell.tsx · "
                  ...
                  "docs/feature-posting.md §0-V · "
                  "/verify 2026-09-06 (커밋 c2ac6f3 — 정적 4/4 + 픽셀 실측)"),
    ))
    return t
    ...
    if off is None:
        ...
        t.gates.append(Gate(
            ...
            + ". ⚠ 2026-08-23 **이웃 업종 분포 피처(8열) 시도 — 개선 없음.** "
              ...
              "진행률 평균에서는 뺀다. 다음 레버는 피처가 아니라 **off-prior 표본 "
              "확대**(거점 추가 또는 라벨 세분화)이고, 이는 별도 스코핑 과제다. "
              "⚠ **2026-09-06 스코핑 완료 — 두 축의 값이 갈렸다**(docs/scope-offprior-sample-2026-09-06.md · "
              "reports/offprior_sample_scoping_2026-09-06.json). **먼저 인용된 877 이 낡았다**: "
              "54거점 시절 값이고 66거점인 지금 세면 **1,011자리**다(그래프는 안 낡았다 — "
              "노드 47,442 · district_id 고유 66 · 09-04 빌드. 낡은 것은 `load_graph()` "
              "독스트링의 '33거점'과 이 877 뿐). 877 → 1,011 이면 표준오차 1.63 → **1.52%p** 라 "
              "'3~4%p 아래는 못 가른다'는 08-26 결론은 **그대로 살아 있다**. "
              "**① 라벨 세분화 — 표본 4.35배, 그리고 이미 구현돼 있다**: `--label-level "
              "category2`(train_gnn.py 에 이미 있다). off-prior 자리 1,011 → **4,399**"
              "(test 중 10.6% → 46.3%) · 표준오차 1.52 → **0.73%p** · 가별 최소 차이 "
              "3.05 → **1.46%p**. 크게 느는 이유는 `_offprior_top3` 독스트링이 이미 적어 둔 "
              "그대로다 — 라벨이 7종이면 거점 상위 3이 test 의 89.4%를 덮어 남는 자리가 "
              "10.6%뿐인데, 32종이면 46.3%가 남는다. ⚠ **서빙 라벨 교체가 아니다** — "
              "`train()` 이 label_level != 'group' 이면 산출물 저장을 강제로 끈다(train_gnn.py:826). "
              "category2 는 **계측 도구**이고, 32종에서 유의한 레버가 7종에서도 유의하다는 "
              "보장은 없다(증거이지 증명이 아니다). "
              "**② 거점 추가 — 이 문제의 답이 아니다**: 거점당 off-prior 15.3자리이고 "
              "표준오차는 1/√n 이라 **절반으로 줄이려면 거점 약 264곳**(현재 66)이 필요하다. "
              "보류 중인 고양·파주 20거점을 다 켜도 +306자리(1,011 → 1,317) · 표준오차 "
              "1.52 → 1.33%p 라 **08-26 이 못 가른 +2.05%p 를 여전히 못 가른다**. "
              "→ **권고: 라벨 축만 간다.** category2 대조군 1런 뒤 기각된 네 레버"
              "(이웃·행정동·집계구·조기종료)를 같은 날 같은 코드로 재검정하고, 유의한 것이 "
              "나오면 그때 7종에서 다시 확인한다. 따로 돌린 값을 대면 안 된다 — 08-26 이 "
              "기준선이 낡아 행정동 기각이 −0.12 에서 +1.59%p 로 뒤집힌 자리다.",
            observe=True,
        ))
    return t
...
def posting_track(total: int) -> Track:
    ...
    t.gates.append(Gate(
        ...
        + (f"(거점 내 서열까지 실측 {foot_measured}/{total} ✅ — **집계구 생활인구**. "
           ...
           f"— 게이트가 통과한다고 입도가 같은 것이 아니다. 원인은 수집이 아니라 "
           f"**낡은 산출물**이었다: `silver/unit_jipgyegu.json` 이 08-25 빌드라 거점 54 "
           f"에 멈춰 있었고(빌더 자체는 glob 이라 멀쩡했다), 한 번 돌리니 유닛 528 → "
           f"**664** · 거점 54 → **66** 이 됐다. 기존 **528/528 값은 그대로**이고 136 "
           f"유닛이 더해진 순수 상위집합이다. 이제 12거점 136유닛 전부 "
           f"`flpop+jipgyegu` 다. ⚠ **다만 화면에 보이는 변화는 작다** — 원값 고유 수는 "
           f"거점당 중앙 3.5 → **9.0**(2.6배) 로 늘었지만 `foot` 은 등급 하나라 "
           f"방출되는 offset 은 여전히 ~3단계로 양자화된다. 입력이 고와진 것이지 "
           f"출력 해상도가 올라간 것이 아니다) · "
    ...
    t.gates.append(Gate(
        ...
        f"⚠ **2026-09-06 — 이 자리가 '다음 레버는 상가정보 `flrNo` 로 유닛별 실면적을 "
        f"특정하는 것이고, 그것이 열리면 이 값이 1.0 으로 오른다' 고 적고 있었다. 재 보고 "
        f"기각했다**(§0-W, reports/posting_unit_area_flrno_probe_2026-09-06.json). "
        f"⚠ 이 줄은 **같은 날(09-05) 쓰인 §0-T 와 이미 어긋나 있었다** — §0-T 가 층 단위 "
        f"실측 면적 목록을 내면서 'ROI 표본은 건드리지 않았다'고 못박았는데, 이 줄은 그 "
        f"자료로 표본의 면적을 고칠 수 있다는 뜻으로 읽혔다. 한 출력 안에서 두 자리가 "
        f"다른 말을 하는 이 저장소의 주된 실패 양식이다. "
        f"**재료는 또 이미 있었다** — `flrNo`·층번호는 `build_building_attrs` 에 배선돼 "
        f"있고, 층별 **면적**은 층별개요 원본의 `area` 필드에 수집돼 있으며, 층 단위 공실 "
        f"유닛은 `gold/*/vacant_floor_units.json` 에 66거점 **12,497개**(confirmed 9,437 / "
        f"probable 3,060) 이미 빌드돼 있다. 막는 것은 수집이 아니다. **막는 것은 유닛의 "
        f"정의다**: 유닛을 단일 층으로 확정할 수 있는 것은 664개 중 **30개(4.5%)** 뿐이고, "
        f"그 30개는 **정확히 `capacity == 1`** 인 유닛이다(단일층 ⟺ capacity 1, 양방향 "
        f"완전 일치 30/30). 그런데 capacity 1 이면 `com_area / capacity` 가 이미 그 한 층의 "
        f"면적이라 **델타 0평**(min=max=median=0 · 비 median 0.999). 즉 **적용할 수 있는 "
        f"곳에서는 아무것도 안 바뀌고, 바뀔 곳(95.5%)에는 적용할 수 없다.** 서빙 유닛은 "
        f"건물 공실용량을 **호실당 평균 1개**로 싣기 때문에(664 ↔ 층유닛 12,497) 층 면적을 "
        f"주려면 유닛을 층마다 쪼개야 하고, 그것이 곧 §0-Q(528→2,201)라 프라임 프리미엄 "
        f"부호가 뒤집힌다. **`flrNo` 는 §0-Q 의 독립 후속이 아니라 같은 것이다** — 분할에 "
        f"층 라벨을 더 정확히 붙일 뿐 분할 자체를 피하게 해 주지 않는다. ⚠ 잃는 입도는 "
        f"실재한다(다층 건물 3,671동 · 건물 내 층 면적 최대/최소 비 중앙 **1.177배** · p90 "
        f"**3.416배** · 48.3%가 1.2배 초과) — 그래서 아깝지만 집는 길이 하나뿐이다. "
        f"**남은 조건은 수집이 아니라 §0-Q 의 사전등록 임계값을 넘기는 모델 재설계**"
        f"(층 분할 후에도 프리미엄 부호가 유지되도록 임대료 층 계수를 다시 세우는 일)이고, "
        f"별도 스코핑 과제다.",
        evidence=("data/pipelines/build_vacant_units.py (area_pyeong) · "
                  ...
                  "docs/finding-posting-unit-area-flrno-2026-09-06.md · "
