...
/**
 * SpaceOS 프론트엔드 진입점.
 * 도시 전환: 고양시(거점 대시보드 ↔ 거점 PPPP 심층) ↔ 서울(25구 로드맵) ↔ 가로수길 Page(지도-우선).
 * 고양 대시보드에서 거점 클릭 → districtId 전달 → 해당 거점 심층 뷰 오픈.
 * TODO: react-router 도입 시 /goyang, /goyang/d/:id, /seoul, /gangnam 로 분리.
 */
type City = "goyang" | "seoul" | "gangnam";
type GoyangView = "dashboard" | "district";

export default function App() {
  const [city, setCity] = useState<City>("gangnam");
  const [view, setView] = useState<GoyangView>("dashboard");
  const [districtId, setDistrictId] = useState("lafesta");

  return (
    // 풀높이 flex: 네비바(고정) + 콘텐츠(나머지 전체). MapShell 이 콘텐츠 영역을 꽉 채운다.
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column" }}>
      <nav style={{
        ...
        <NavBtn label="가로수길 Page" active={city === "gangnam"} onClick={() => setCity("gangnam")} />
