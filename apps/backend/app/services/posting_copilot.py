...
PlaceOS 가 계약을 발행하고 공급자가 여기에 맞춘다.** 어댑터는 오늘 구현되고, 계약을
...
# ⚠ 와이어 계약 문자열 — 2026-09-12 브랜드 개명(SpaceOS → PlaceOS)에도 그대로 둔다.
#   공급자가 이 값으로 맞춰 오므로 이름을 바꾸려면 계약 버전을 올려(/2) 양쪽을 함께 옮겨야 한다.
CONTRACT = "spaceos.posting/1"
_TIMEOUT_S = 8
