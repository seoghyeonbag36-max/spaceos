...
#
# ⚠ 위 두 문단에 7 · 17 이라는 서로 다른 수가 나온다 — **셋 다 맞고, 세는 대상이 다르다.**
#   09-05 에 이 주석만 읽고 "보류 거점이 7곳"으로 잘못 읽어서 여기 적어 둔다:
#     · **20** = 등록된 비서울 거점 전부 (고양 14 · 파주 6). 보류로 빠지는 것은 이 20 이다
#     · **7**  = 그중 Gold·앵커까지 서 있는 거점 (geumchon·hwajeong·ilsan·tanhyeon·
#                unjeong·westerndom·yadang) — 되넣으면 **그날로 화면에 뜨는** 것들
#     · **17** = 되넣었을 때 아직 수집이 남는 거점 (대장 13 + 공실유닛 4)
#   세는 것은 `python scripts/chain_status.py --all` 이다. 이 수들을 손으로 갱신하지 말 것.
#
# **이 상수는 열린 질문이 아니라 내려진 결정이다.** `/pppp-status` 가 보류를 물어 오면
# 그건 결정이 없어서가 아니라 요약이 그렇게 읽히기 때문이다 — 되넣기 전에 필요한 것은
# 판단이 아니라 **경기 상권을 서빙할 사업적 이유**다(있으면 도시 id 한 줄).
