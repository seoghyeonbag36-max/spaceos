...
⚠ **이 표는 PoC 착수기(2026-05~07)의 우선순위다.** 지금 서빙하는 거점은 **서울 66곳
전부 Tier1**(건축물대장 실측)이고, 목록의 단일 출처는 `data/config/page_hubs.ACTIVE_HUBS`
(= `measured_pages.SERVED_CITIES` 필터)다. 아래 셋은 그 66 안에 들어 있고, 여기 없는
거점이 63곳 더 있다 — **이 표를 거점 목록으로 읽지 말 것**. 수는
`python scripts/pppp_status.py` 로 센다(고양·파주 20거점은 서빙 보류라 세지 않는다).
