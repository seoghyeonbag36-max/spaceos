...
| **거점** | **서울 66거점 전부 Tier1**(건축물대장 실측 · 08-17 에 54 완주, 이후 3차 12거점 추가). PoC 출발점은 신사동 가로수길 — **숫자를 여기서 읽지 말 것**, 거점 수는 `data/gold/*/coverage.json` 의 `tier` 를 세는 것이 단일 기준이고 `python scripts/pppp_status.py` 가 그걸 찍는다(이 줄은 08-02·09-05 두 번 낡았다). 고양·파주 20거점은 **서빙 보류**(`measured_pages.SERVED_CITIES={seoul}`) — 게이트가 세지 않는다 |
