...
> ⚠️ **이 런북은 완주된 이력이다 — 그대로 다시 돌리지 말 것 (2026-08-17 확인).**
> 아래 "13거점"은 작성 시점 값이고, 현재 LSTM 은 **27거점 pooled**
> (`vacancy-lstm-pooled-v2`)로 학습·서빙 중이며 거점 자체는 **54곳**이다. §11 의
> "기존 gold 를 덮어쓰지 말 것"이 특히 중요하다 — 지금 실행하면 27거점 산출물을
> 13거점본으로 되돌릴 수 있다.
>
> 현재 상태는 `python scripts/pppp_status.py` 로 확인하고, Platform 작업은 `/platform`
> 으로 들어간다. 남은 게이트는 학습이 아니라 **Top-1 70% KPI 재정의 판단**이다
> ([docs/feature-platform.md](../../docs/feature-platform.md) — 세분 라벨·균형 손실 모두 실측 기각).
