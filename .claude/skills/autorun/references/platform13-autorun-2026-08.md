...
> ⚠️ **이 런북은 완주된 이력이다 — 그대로 다시 돌리지 말 것 (2026-08-20 확인).**
> 아래 "13거점"은 작성 시점 값이다. 현재 LSTM 은 **54거점 pooled**
> (`vacancy-lstm-pooled-v2`, 07-25 학습)로 학습·서빙 중이고, GNN 은 **105열 40,388노드**
> (08-19 서빙본)다. §11 의 "기존 gold 를 덮어쓰지 말 것"이 특히 중요하다 — 지금 실행하면
> 54거점 산출물을 13거점본으로 되돌릴 수 있다.
>
> 현재 상태는 `python scripts/pppp_status.py` 로 확인하고, Platform 작업은 `/platform`
> 으로 들어간다. Top-1 70% 게이트는 **08-17 에 폐기**됐다. 남은 게이트는
> **off-prior Top-3 ≥50%**(현재 37.6%)이며 학습 반복이 아니라 **새 피처**가 선행이다
> ([docs/feature-platform.md](../../../../docs/feature-platform.md) §0-B~0-D).
