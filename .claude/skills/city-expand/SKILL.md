...
| 행안부 LOCALDATA · KOSIS · 공정위 · SGIS | 개폐업·매출앵커·공간단위 |
...
| 서울 인허가 | **경기데이터드림 `GENRESTRT`** — LOCALDATA 는 폐쇄(실측), 표준데이터 파일 403, data.go.kr 미러는 LINK형 |
...
| `data/config/rone_districts.py` | 경기 3매핑 + `SHARED` 표기 | ⬜ |
| `data/collectors/gg_*.py` | 경기 소스 수집기 — **시군 파라미터를 받는 하나**로 | ⬜ |
...
| 1 | 거점 등록 → 수집 → 공실 → 지도 (`hub-onboard`) | 거점별 `coverage.json` **Tier1** · 앵커 대조 가능 |
