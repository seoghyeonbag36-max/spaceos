...
물리적 상권을 디지털 트윈으로 플랫폼화하기 위한 **첫 단계**: 서울 25개 구를 4기준(SNS·공실·지도유입·유동인구) × 2대상(B2C 방문고객·B2B 입주업체)으로 채점해 PlaceOS 진입 우선순위를 산출한다.
...
2. **Silver**: `SCORE_BANDS`로 대상별 1~4점 변환. 공실(VAC)은 B2B에서 "많을수록 高점수"(PlaceOS가 푸는 문제), B2C에서는 상한 3으로 제한.
