...
⚠️ **2026-08-01 현재 백엔드에 배선되지 않았다 — 근거 부재로 보류다.** 산출물은 13거점 ×
12유닛으로 검증됐지만(`gold/{slug}/vacant_units.json`), `/postings` 는 아직 시드 유닛을 쓴다.
`tier_scenarios()` 가 요구하는 세 입력이 실제 건물 유닛에 없는데, **무엇으로 대신할지
정할 자료 자체가 없다.** 결정을 미룬 게 아니라 결정 근거가 없는 상태다.

  1. `prem`(권리금) — **데이터 부재.** 공개 통계가 없다(bronze 전수 확인). 0 이면 초기투자
     과소, 임의 추정치면 그 위 ROI 가 전부 가정이 된다. 어느 쪽이 덜 틀린지 판단할
     기준이 없다. → 권리금 소스 확보, 또는 "시뮬레이션이 권리금을 포함해야 하는가"라는
     제품 판단이 있어야 풀린다.
  ...
  3. `rec`(추천 Tier) — **기준 정의 부재.** ROI 최단으로 계산하면 defensible 해 보이지만
     "추천"이 무엇을 최적화하는지가 정의된 적이 없다(회수 최단 / 순익 최대 / 리스크 최소에
     따라 답이 갈린다). → 추천 기준의 제품 정의가 먼저다.
...
- `prem` 권리금: 공개 통계가 없다(bronze 전수 확인). 거점 중앙값 기반 추정치를 넣고
  `inputs_source.prem = "seed"` 로 표기한다.
    ...
    for slug in slugs:
        ...
        out = {
            ...
            "note": ("면적은 건물 상업면적 ÷ 호실 수(호실당 평균, 평). 대장 면적이 없는 "
                     ...
                     "층은 1F 가정(상가정보 flrNo 매칭 전). 권리금은 공개 통계가 없어 유닛에 없다."),
            "units": units,
        }
