...
## 소분류 어휘는 실측을 **시도했고 막혔다** (2026-09-15)

종전 머리말은 "소분류 어휘는 아직 실측하지 못했다"였다. 그 실측을 시도한 결과를
경위로 남긴다 — 결론은 바뀌지 않았지만, **왜 못 하는지**가 근거로 고정됐다.

`--audit` 를 전 거점에 돌린 출력(`reports/logs/taxonomy_audit_20260915.log`):

    [taxonomy] 거점 66곳 · 행 0 · 가두 0
    [taxonomy] ⚠ Bronze 없음 66곳: garosugil, apgujeong-rodeo, hongdae, …
    [taxonomy] 셀 것이 없다 — building_vacancy 로 stores_raw 수집이 먼저다

**66거점 전부 Bronze 없음**(`.gitignore:27` — 신규 클론은 구조적으로 못 갖는다).
지시된 폴백(`building_vacancy` 선행)도 막혔다: `DATA_GO_KR_SERVICE_KEY` 미설정 ·
`data/.env` 부재로 수집기가 스스로 건너뛴다. 머신 전체에 `stores_raw*` 0건.

그래서 **`SCLS_*` 상수를 추가하지 않았다.** 넣을 실측 문자열이 0개다.
`편의점`·`약국`·`커피전문점` 셋 다 "이 표본에 없다"가 아니라 **"표본이 없다"** 다.

⚠ **함정 — 소분류처럼 보이는 것이 저장소에 있다. 그것은 카카오다.**
`gold/*/program_content_context.csv` 의 `kind=category` 행은 빌더상 정확히 소분류인데
(`build_gold.py:524` 가 `category_path(...).split(" > ")[-1]` 로 만든다), **커밋된
파일은 소스 교체 이전 산출물**이다(CSV `c7a51df` 2026-09-04 ↔ 교체 `8432cbc`
2026-09-15). 66파일 124종 전수 집계가 그것을 증명한다 — 슬래시 결합 **0종**(상가정보
형태), 쉼표 결합 17종(카카오 형태), 그리고 `CU`·`GS25`·`스타벅스` 같은 **브랜드명
5종**. 247개 고정 코드북에 상호가 있을 수 없다. 거기서 `약국`(62)·`커피전문점`(58)을
집어다 "실측"이라 부르면 **약관 때문에 걷어낸 카카오 어휘를 규칙으로 되돌리는 것**이다.

후보 다섯 곳을 전부 열어 본 기록과 재개 절차(자격증명 → 수집 → 감사):

    docs/finding-store-taxonomy-scls-2026-09-15.md

⚠ 따라서 세 규칙(`scls` 의 "편의점"·"약국"·"카페/커피/다방/찻집")은 **여전히
미검증 추측**이다. 값이 안 맞으면 그 셋을 먼저 의심할 것. `test_store_taxonomy.py`
가 드는 `커피전문점/카페/다방`·`편의점`·`약국` 도 같은 추측이라, 실측이 되는 날
**가장 먼저 대조할 대상**이다(규칙과 테스트가 같은 추측을 공유하면 서로를 검증하지
못한다).
"""
