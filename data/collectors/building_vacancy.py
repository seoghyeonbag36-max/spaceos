...
# 2026-08-23 확장: 고시원·독서실·다중생활시설·다중주택·다가구주택. 앞의 숙박·공장 건과
# **완전히 같은 종류의 오염**이다 — 용도상 근린생활시설이라 분모에 들어오지만 분자
# (상가정보)는 이들을 점포로 싣지 않아 구조적으로 빈 층이 된다. 녹두거리(고시촌)에서
# 극단적으로 드러났다: 2층 이상 분모 903행 중 독서실 269 · 고시원 137 · 다중주택 66 ·
# 다가구주택 46 = 60% 이고, 그 결과 층별 공실이 1층 0.0% 인데 3층 74.0% · 5층 90.2% 로
# 상층부만 무너져 앵커 격차가 +34.6%p(54거점 최대)까지 벌어져 있었다. 같은 신림역 앵커를
# 쓰는 sillim 이 +19.6%p 인 것이 앵커가 아니라 분모의 문제임을 가른다.
# ⚠ "학원"은 남긴다 — 상가 임차 단위가 맞고 상가정보에도 점포로 실린다(판단 근거는
# docs/feature-page.md 참조). 주거류는 이 5종으로 전수 커버된다(57거점 bronze 점검:
# 추가 후 분모에 남는 주거 용도 0건).
# 2026-08-23 확장 ②: 호스텔·금융업소. 앞의 확장들과 판단 기준이 같다 —
# **분자가 그 용도를 볼 수 있는가**(지상 2층↑ 상업층 중 점포·인허가로 확인된 층의 비율).
#   · 호스텔 876층(1층 71 · 2층↑ 805, 확인율 37.0%) — 같은 숙박 계열인 관광호텔 6.5% ·
#     여관 3.6% · 호텔 7.5% · 기타일반숙박시설 5.1% 은 07-28 에 이미 뺐는데 필터 문자열에
#     "호스텔"이 없어 이것만 남아 있었다. 305실 호텔이 점포 1건으로 세어지던 것과 같은
#     오염이다(숙박업 인허가가 그 층을 확인해 줘도 '업소 1건'이라 분자는 1이다).
#   · 금융업소 871층(1층 282 · 2층↑ 589, 확인율 22.9%) — 걷어낸 무리(≤24%)와 같은 대역이다.
#     은행 지점은 **상가업소정보에 대분류 자체가 없고**(garosugil 5,264행 전수: 금융 0건)
#     지방행정 인허가에도 없어 분자가 구조적으로 못 본다.
# 이 필터가 안전한 이유: 분모는 `capacity_floors()` 에서 **층별개요 상업층 ∪ 점포·인허가
# 확인층**이다. 용도를 빼도 근거가 있는 층은 도로 들어오고, 실제로 빠지는 것은 분자가
