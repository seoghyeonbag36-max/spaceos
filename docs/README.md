...
| [prd-landing.md](prd-landing.md) | **랜딩 PRD (D2 산출물)** — Lovable 에 그대로 넣는 입력본. 섹션 S0~S11 순서·카피 초안·이미지 슬롯·CTA·모바일 접힘 규칙. 독자는 투자자 하나로 고정(부록 C 로 교체) |
...
- 성능 목표: **AI 는 '정확도 70%+' 를 2026-09-16 에 폐기**하고 베이스라인 대비 실력으로 간다 (무정보 규칙이 그 선을 이미 넘었다 → [finding-kpi-leak-2026-09-16.md](finding-kpi-leak-2026-09-16.md)). 지도·건물 상세 로딩 <3초 · API p95 <200ms 는 2026-09-16 에 계측기를 세웠다(`GET /api/v1/admin/latency`) — 표본이 적으면 계측기가 스스로 판정을 보류한다. 배선 ≠ 달성이다
