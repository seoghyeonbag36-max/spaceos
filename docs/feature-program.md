...
1. **자리(Site)** 자동 — `unit_id` `lat/lng` `area` `floor` `was`(직전 업종) `건물 공실률`
   출처: `vacant_units.json`(**49거점 524유닛**) → `page_building_master.geojson`(54거점) 폴백
...
| 4 | 공실 유닛 인벤토리 ~~13/54~~ → **49/54** | ✅ **재료는 확보됐다 (2026-08-16)** — 잔여 5곳도 막힘이 아니라 `build_vacant_units` 재실행 대기다(2026-08-17 확인). **막는 것은 이제 인벤토리가 아니라 ①층이 이 파일을 읽지 않는다는 것** — 수집이 아니라 배선 과제로 성격이 바뀌었다 |
