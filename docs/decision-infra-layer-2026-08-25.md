...
> ⚠ **2026-09-15 정정 — 아래 표는 2026-08-25 의 스냅샷이고 네 행이 그 뒤에 바뀌었다.**
> 지우지 않는 이유는 §2~§4 의 결정이 **이 표를 근거로** 내려졌기 때문이다. 다만 이 표를
> "지금의 실태"로 인용하면 틀린다 — 선언이 낡는 것이 이 저장소의 주된 실패 양식이라
> 여기에도 같은 정정을 붙인다.
>
> | 행 | 08-25 스냅샷 | 지금 |
> |---|---|---|
...
>
> 나머지 행(PostgreSQL/PostGIS·Redis·Celery 미사용 · Airflow DAG 미가동 · 과금 스텁)은
> **그대로다**(2026-09-15 재확인).
...
재발 방지는 `.github/workflows/ci.yml` 의 **`최소 의존성 임포트` 잡**이다(당시 이름은
`서버리스 임포트`). 루트 `requirements.txt` **만** 깔고 `api/index.py` 를 import 한
뒤 익명 요청 200 을 확인하고, 마지막에 **psycopg2 가 없음**까지 확인한다 — 누가 그 최소 셋에
