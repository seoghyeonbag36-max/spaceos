...
- **유동인구 흐름**: `naver.maps.visualization.HeatMap`(생활인구 밀도) + **시간 슬라이더 애니메이션**(시간대별 값 재생 = 흐름 근사). ~~진짜 이동 벡터(OD 유선)~~ → 생활이동으로 확장되는 것은
**유입 흐름**(출발지→거점 집계)이지 이동 벡터·동선이 아니다 — 원천에 개인 축이 없다
(2026-09-17 검증 6회, [finding-movement-viz-2026-09-17.md](finding-movement-viz-2026-09-17.md)).
...
목표 지역은 **서울 강남구**. 강남구 주요 상권을 PlaceOS 선정 기준(데이터 가용성 + 공실 신호 + 경계 명확성 + B2B 접근성)으로 비교한 결과 **신사동 가로수길**로 확정한다.
...
2. **거시 지표가 실체를 숨기는 사례** — 가두상권 공실률 **41.6%** vs 신사역 집합상가 **9.99%**. 거시 통계로 안 보이는 건물별 실상을 디지털 트윈이 드러낸다는 PlaceOS 핵심 가치제안을 그대로 증명.
...
> ⚠️ **거시 정합 기준 정정 (2026-07-28).** 위 41.6% 는 **부동산원 통계가 아니다** —
> §0.5 상권 비교표의 출처인 가로수길 **가두 1층 실태조사**(2024, 언론 인용) 값이다.
> 같은 상권의 실제 R-ONE 값은 **신사역 중대형 17.6% / 소규모 14.9%**(2026Q1)이고,
> 이 값들은 `bronze/platform13/*/rone_vac_*.json` 에 이미 수집돼 있다.
> 2026-07-19 의 "거시 -2.5%p 로 exit 통과" 는 **서로 반대 방향의 오차 두 개가
> 상쇄된 결과**였다: ① 분자가 업종 필터 이전 값이라 부풀어 추정치를 끌어내렸고
> (57.4% → 38.3%), ② 앵커가 실제보다 24%p 높았다. 둘 다 고친 뒤의 거시 격차는
> **+36.6%p** 다. 따라서 **거시 정합은 아직 통과하지 못했다**.
> 로드뷰 라벨 대비 status 정확도 75% (기준 70%+)는 유효하다 — 이쪽은 분자·분모의
> 절대 수준이 아니라 건물 간 상대 순서를 보는 지표라 위 두 오차의 영향이 작다.
> → 거시 정합은 α보정(§3-1)을 거친 값으로만 주장할 것. 원시 집계치로는 주장 불가.
