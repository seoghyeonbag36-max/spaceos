...
토큰은 CSS 변수 체계로만 간다. Tailwind·Storybook 을 설치하지 마라
(설치도 안 된 채 설정만 남아 있던 tailwind.config.ts 와 .storybook/ 은 2026-09-06 에 삭제했다).
...
## 6. 품질 — 접근성·일관성 검수
```
모든 컴포넌트의 대비(AA)·포커스 링을 직접 점검하고(Storybook a11y 애드온은 없다),
