...
| 독립 표본 감사 | 확률표집, 독립 근거, 검토자 판정과 미판정 기록 | 검토 표본 추출 완료. 독립 사람 판정은 미실행 | [표본 설계](audits/page-analysis-20260906/sample-design.json) |
| 영향·재현성 분석 | 같은 단위·시점의 영향 비교, 고정 입력 재실행과 릴리스 차이 분리 | 층 범위·행 순서 분석 및 현재 마스터·서빙 반복 재현 완료. 독립 오류 정정 효과는 미평가 | [분석 보고서](audits/page-analysis-20260906/README.md) |
...
manifest의 선택 경로를 사용한 구조·영향·재현성 분석 결과는 위 보고서에 있다. 다음은 독립 검토 자료와 사람 판정의 확보 및 검증된 결과의 근거 등재다. 변경된 입력이 있으면 이전 실행에 섞지 않고 새로운 run-id로 재포착한다. 자동 분석 결과를 현실 공실 정확도나 과거 실행의 완전 재현으로 확대하지 않는다.
