...
- 산출: `ml/artifacts/industry_gnn.pt` + `data/gold/platform_industry_recommend.json`(40,388노드 Top-3, ~8.0MB — 2026-08-16 수요신호본 기준) + `ml/mlruns`. 서빙: `apps/backend/app/services/industry_recommend.py`(좌표→최근접 노드 400m, 없으면 거점 평균) → `/api/v1/ai/recommend-industry`(스텁 교체 완료). json 은 .gitignore/.vercelignore 예외로 배포 포함.
...
⚠ **서빙 산출물은 아직 95열이다**(위는 `--no-save` 런). save 런 한 번이면 105열로 바뀌고
게이트가 자동 전환되지만, `/recommend-industry` 응답이 달라지므로 별도 판단이다.
