...
🔴 **현행 모델(26.45%)이 자리를 전혀 안 보는 값싼 규칙(42.42%)에 진다** — 이 능력은
'거의 다 된' 게 아니라 미구축이다. 올릴 레버는 **Page 의 층·용도**(편의점 1층/병원 상층부)와
**Platform 의 리뷰·감성**(문화시설), **건물·인접 구조**(약국↔병원)이고 셋 다 미배선이다.
지표 배선은 `_offprior_top3()` 로 끝났고, 다음 학습 런부터 산출물에 자동 기록된다.
**Platform 진행률 97.8% → 88.2%.**
