...
### 유동 레이어 — 소스 조사 완료, 착수 대기 (2026-08-23)

배선하려다 재료부터 확인했고, **경로가 정해졌다.** 다음 세션이 다시 조사하지 않도록 남긴다.

- ⚠ **시간 슬라이더가 아무 데도 연결돼 있지 않다.** `MapShell.tsx` 의 `hour` state 는
  존재하지만 오버레이 effect 의 의존성 배열에 없어서 **드래그해도 지도가 안 바뀐다.**
  데이터와 별개인 독립 결함이다.
- 기존 TRDAR `seoul_trdar_flpop` 은 **상권 단위 총량**이고 좌표가 없다 — 히트맵 격자로
  직접 못 쓴다. 시간대도 6구간뿐이라 24시간 슬라이더와 눈금이 안 맞는다.
...
신설할 것: `collectors/living_pop_dong.py` · `pipelines/build_footfall.py` ·
`api/v1/heatmap.py::footfall` · `MapShell` 슬라이더 실연결.
