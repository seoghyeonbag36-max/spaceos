...
> ⚠ **표기 규칙 (2026-09-17 확정)** — 이 트랙이 주는 것은 **유입 흐름**이지 **이동 동선**이
> 아니다. 원천 열 10개 중 사람·트립을 잇는 축이 **0개**라 궤적(A→B→C)은 가려진 것이 아니라
> **존재하지 않고**, 집계에서 복원되지도 않는다(2×2 최소 표에서도 해가 3가지). 거점은
> 행정동보다 작아 **내부 이동은 노드 1개로 접힌다**. "동선·궤적·경로"라고 적지 말 것.
> → [finding-movement-viz-2026-09-17.md](finding-movement-viz-2026-09-17.md) (검증 6회 ·
> 단일 기준 `python scripts/probe_movement_viz.py`)
